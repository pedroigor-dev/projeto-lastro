...
public class ChangeSpecController {
    ...
    ReadinessReport readiness(@PathVariable UUID id) {
        ...
        sample.stop(meters.timer("vertex.readiness.evaluation"));
        meters.counter("vertex.readiness.result", "ready", Boolean.toString(report.ready()))
                .increment();
        return report;
    }
